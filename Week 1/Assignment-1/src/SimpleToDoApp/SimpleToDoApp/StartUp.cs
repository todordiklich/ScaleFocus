...
namespace SimpleToDoApp
{
    public class StartUp
    {
        ...
        private static List<int> EnterUserIds()
        {
        ...
        private static void AssignUsersToTask(ToDo toDo)
        {
            ...
            List<int> userIds = EnterUserIds();

            if (userIds.Any())
            {
                HashSet<int> allUsersId = _userService.GetAllUserIds();
                int counter = 0;
        ...
        private static void ShareToDoList(int toDoId)
        {
            ...
            foreach (var userId in userIds)
            {
                ...
                {
                    Console.WriteLine("You can't share your project with yourself.");
                    continue;
                }
        ...
        private static void EditToDoListById(int toDoId)
        {
            ToDo toDoListToEdit = _toDoService.GetToDoById(toDoId);
        ...
        private static void DeleteUserById(int userId)
        {
            ...
            if (isDeleted)
            {
                ...
                _toDoService.RemoveNotSharedToDoListsWhenDeleteUser(userId);
            }
            else
            {
                Console.WriteLine("There is no user with such Id.");
