...
namespace SimpleToDoApp.Services
{
    public class ToDoService
    {
        ...
        public bool CreateToDoList(string title, User user)
        {
            ...
            int newUniqueId = GenerateToDoId();
        ...
        {
            if (_applicationToDos.Any())
            {
                return _applicationToDos.Max(t => t.Id) + 1;
            }
        ...
        public void RemoveNotSharedToDoListsWhenDeleteUser(int userId)
        {
            List<ToDo> notSharedToDoLists = _applicationToDos.Where(t => t.CreatorId == userId && t.SharedWithUsersIds.Count == 0).ToList();

            foreach (var notSharedToDoList in notSharedToDoLists)
            {
                _applicationToDos.Remove(notSharedToDoList);
            }
